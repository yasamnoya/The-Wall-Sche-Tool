  ...
  for (let i = startOfDate, date = 1; i < length; i++, date++) {
    ...
    if (isWorkDay(block)) {
      ...
      let StartDate =
        ...
        beginTime.getDate();
      let StartTime =
        to2Digits(beginTime.getHours()) +
        ":" +
        to2Digits(beginTime.getMinutes());
      let EndDate =
        endTime.getFullYear() +
        "/" +
        (endTime.getMonth() + 1) +
        ...
        endTime.getDate();
      let EndTime =
        to2Digits(endTime.getHours()) + ":" + to2Digits(endTime.getMinutes());
      personalSche.push([
        EventName,
        StartDate,
        StartTime,
...
function getNameList(inputCSV) {
  ...
  try {
    ...
    var AssistList = possiblyNameList.slice(
      indexOfAssist + 1,
      indexOfNumberOfTBs
    );
